...
const CustomerModal = ({ isOpen, onClose, customer }) => {
    const { register, handleSubmit, reset, formState: { errors, isSubmitting } } = useForm({
        defaultValues: customer || { name: '', phone: '', email: '', address: '' }
    });
    const queryClient = useQueryClient();

    React.useEffect(() => {
        reset(customer || { name: '', phone: '', email: '', address: '' });
    }, [customer, reset]);
...
const Customers = () => {
    ...
    <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">
        Showing customers {paginatedData.length > 0 ? (currentPage - 1) * rowsPerPage + 1 : 0} to {(currentPage - 1) * rowsPerPage + paginatedData.length} of {totalRecords} entries
    </p>
